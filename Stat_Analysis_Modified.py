...
'''
sql_acct_0 = "(组合名称 == {!r} and 证券代码 not in {!r} and 证券代码 not in {!r}) or 组合名称 in {!r}".format('股票组合1',code_1,code_2,['权益类2号组合','权益类3号组合','股票组合3'])
...
#cols = ['组合名称', '证券代码', '数量', '成本价','成本', '浮动盈亏','投资收益', '累积结转', '累积投资收益', '买入费用', '卖出费用']
    ...
    print('计算市值：{:.2f}, O3内市值：{:.2f}'.format(others['市值'].sum(), in_sht_mkt_val))
    ...
    sql_acct_comb = "组合名称 in ('股票组合1', '权益类2号组合', '权益类3号组合', '权益类5号组合', '权益类6号组合')"
    sheet_2 = sheet_1.query(sql_acct_comb).groupby(['证券名称','证券代码'])[['数量','成本','市值','累积结转','累积投资收益','总盈亏','总费用','费后总盈利'] + list(out_cols[-2:])].sum().query("数量 > 0").reset_index()
    ...
    # 总股本/股本占比
    extra_cols = ['证券代码', '总股本', '证券类别', '持仓']
    extra_df = tmp[extra_cols].query("(证券类别 in ['股票', '开放式基金']) and (持仓 > 0)").iloc[:,:2]
    extra_df.drop_duplicates(subset=['证券代码'], inplace=True)
    sheet_2 = pd.merge(sheet_2, extra_df, on=['证券代码'], how='left')
    sheet_2['持仓/总股本'] = sheet_2['数量'] / sheet_2['总股本']
...
Clear_Range(wb['权益类多头汇总明细表'],'A2:P{}'.format(wb['权益类多头汇总明细表'].max_row))
Write_DF(wb['权益类多头汇总明细表'],'A2',sheet_2)
...
def Get_Prof_Futures(df,Asset_No,T_Day=True):
    ...
    if Asset_No == '量化':
        M_F = df.dropna().query("资产单元名称 in ('量化场内01资产单元', '量化场内02资产单元')".format(Asset_No))[col_names]
    else:
...
ws['L5'].value += Get_Prof_Futures(futures_analys,'权益类场内01资产单元') - Get_Prof_Futures(L_M_F,'权益类场内01资产单元',False)
...
ws['C29'].value = futures_analys.loc[futures_analys.基金名称 == '自营权益类6号','风险比例1(%)'].values[0] / 100
ws['E29'].value = futures_analys.loc[futures_analys.基金名称 == '自营权益类5号','风险比例1(%)'].values[0] / 100
...
sh_minus_mkt = mkt_df.loc[mkt_df['证券代码'].str.startswith('6')]['市值'].sum()
sz_minus_mkt = mkt_df['市值'].sum() - sh_minus_mkt


def Get_Net_Amt(df,sql_str,buy_dir=True):
    a, b, c, d = df.query(sql_str, engine='python')[['当日卖费用','当日卖金额','当日买费用','当日买金额']].sum().values
    if buy_dir:
        return c + d
    else:
        return b - a
...
for row in wb['可用资金']['D65:E68']:
    ...
    for col in row:
        if col.column == 'D':
            col.value += Get_Net_Amt(Trades,sql_curr,False) + sum(bonus.query(sql_curr, engine='python')['当日红利'])
        else:
...
### sh
sh_mkt_val = sheet_1.query("组合名称 != '沪港通组合' and 证券代码.str.startswith({!r})".format('6'), engine='python')['市值'].sum() - sh_minus_mkt
wb['打新市值统计']['J2'].value = sh_mkt_val

### SZ
sz_mkt_val = sheet_1.query("组合名称 != '沪港通组合' and 证券代码.str.startswith({!r})".format('0'), engine='python')['市值'].sum() + sheet_1.query("组合名称 != '沪港通组合' and 证券代码.str.startswith({!r})".format('3'), engine='python')['市值'].sum() -sz_minus_mkt
